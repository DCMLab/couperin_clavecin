...
.. code:: bash
   ...
   git clone https://github.com/DCMLab/couperin_clavecin.git
...
* `François Couperin – L'art de toucher le clavecin <https://github.com/DCMLab/couperin_clavecin/archive/refs/heads/main.zip>`__


Data Formats
============

Each piece in this corpus is represented by four files with identical names, each in its own folder. For example, the *Allemande* has the following files:

-  ``MS3/00_allemande.mscx``: Uncompressed MuseScore file including the music and annotation labels.
-  ``notes/00_allemande.tsv``: A table of all note heads contained in the score and their relevant features (not each of them represents an onset, some are tied together)
-  ``measures/00_allemande.tsv``: A table with relevant information about the measures in the score.
-  ``harmonies/00_allemande.tsv``: A list of the included harmony labels (including cadences and phrases) with their positions in the score.
...
.. code:: python
   ...
   labels = ms3.load_tsv('harmonies/00_allemande.tsv')
   notes = ms3.load_tsv('notes/00_allemande.tsv')
...
For questions, remarks etc., please `create an issue <https://github.com/DCMLab/couperin_clavecin/issues>`__ and feel free to fork and submit pull requests.
